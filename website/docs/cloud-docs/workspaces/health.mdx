...
- To view [on demand assessments](/terraform/cloud-docs/workspaces/health#on-demand-assessments) for a workspace, you must be an [administrator for that workspace](/terraform/cloud-docs/users-teams-organizations/permissions#workspace-admins).
...
-> **Note:** On Demand Assessments are only available through browser sessions. If the workspace requirements are not satisfied, you should see one or more error messages on the page indicating the blockers to enable health assessments. The button will be disabled until the requirements are satisfied.


If you are an administrator for the workspace and all [workspace requirements](/terraform/cloud-docs/workspaces/health#workspace-requirements) are satisfied, a **Start health assessment** button is available on the Health pages with Ready status.

  ![On Demand Assessments Screenshot: Start health assessment button](/img/docs/on-demand-assessments.png)

You should see a success or error message box in the left-bottom corner of the page, after clicking the button to indicate if the assessment was triggered. The button will be disabled when the assessments are in progress indicating the status. 
Based on factors like the size of the workspace and network latency, the assessment could take several minutes.
