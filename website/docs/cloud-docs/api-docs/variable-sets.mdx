...
description: >-
  Variable sets allow you to reuse variables across multiple workspaces and projects.
  Create, read, update, and delete variable sets with the Terraform Cloud API.
---

# Variable Sets API

A [variable set](/cloud-docs/workspaces/variables#scope) is a resource that allows you to reuse the same variables across multiple workspaces and projects. For example, you could define a variable set of provider credentials and automatically apply it to a set of assigned workspaces, all workspaces in a project, or all workspaces in an organization.
...
| `data.relationships.projects` | array   | \[]     | Array of references to projects that the variable set should be assigned to. Sending an empty array clears all project assignments. |
...
Both of the endpoints support pagination [with standard URL query parameters](/terraform/cloud-docs/api-docs#query-parameters). Remember to percent-encode `[` as `%5B` and `]` as `%5D` if your tooling doesn't automatically encode URLs.
  ...
  "data": [
    {
      ...
      "relationships": {
        ...
        "projects": {
          "data": [
           {"id": "prj-abcd12345", "type": "projects"},
           {"id": "prj-abcd12346", "type": "projects"}
          ]
        }
      }
    }
...
Accepts a list of projects to add the variable set to. When the variable set is applied to a project, all workspaces in the project will have the variable set applied to them.
